...
for(i in 1:length(lake_sites$field_site_id)) {  
  ...
  fit <- lm(site_target$temperature ~ site_target$air_temperature 
            ...
            + site_target$eastward_wind
            + site_target$northward_wind)
  ...
  param_df <- data.frame(beta1 = rnorm(n_members, fit$coefficient[1], params_se[1]),
                       ...
                       beta4 = rnorm(n_members, fit$coefficient[4], params_se[4]),
                       beta5 = rnorm(n_members, fit$coefficient[5], params_se[5]))
  ...
  forecasted_temperature <- param_df$beta1 + param_df$beta2 * site_drivers$air_temperature
                                           + param_df$beta3 * site_drivers$precipitation_flux
                                           + param_df$beta4 * site_drivers$eastward_wind
                                           + param_df$beta5 * site_drivers$northward_wind
                                           + rnorm(n = forecast_horizon, mean = 0, sd = sigma)
